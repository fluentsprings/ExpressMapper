...
namespace ExpressMapper
{
    public sealed class MappingServiceProvider : IMappingServiceProvider
    {
        ...
        public IList<Expression> GetMapExpressions(Type src, Type dest, bool withDestinationInstance)
        {
            var cacheKey = CalculateCacheKey(src, dest);
            if (TypeMappers.ContainsKey(cacheKey))
            {
        ...
        public TN Map<T, TN>(T src, TN dest = default(TN))
        {
            ...
            if (TypeMappers.ContainsKey(cacheKey))
            {
                ...
                // todo : TypeMapper should have one signature for MapTo
                return mapper != null
                    ? ((EqualityComparer<TN>.Default.Equals(dest, default(TN)))
                        ? mapper.MapTo(src)
        ...
        //            .FirstOrDefault(t => t.IsGenericType && t.GetGenericTypeDefinition() == GenericEnumerableType) ??
        //        (typeof (T).IsGenericType
        //         && typeof (T).GetInterfaces().Any(t => t == typeof (IEnumerable))
        //            ? typeof (T)
        //            : null);

        //    tnCol = typeof (TN).GetInterfaces()
        //        .FirstOrDefault(t => t.IsGenericType && t.GetGenericTypeDefinition() == GenericEnumerableType) ??
        //            (typeof (TN).IsGenericType && typeof (TN).GetInterfaces().Any(t => t == typeof (IEnumerable))
        //                ? typeof (TN)
        //                : null);
        ...
        //        return _mappingServices.First(m => !m.DestinationSupport).MapCollection(cacheKey).DynamicInvoke(src);
        //    }
        ...
        public Tuple<Expression, Expression> GetMemberMappingExpression(Expression left, Expression right)
        {
        ...
        internal Tuple<Expression, Expression> GetDifferentTypeMemberMappingExpression(Expression callGetPropMethod, Expression callSetPropMethod)
        {
            ...
            return new Tuple<Expression, Expression>(
                _mappingServices.FirstOrDefault(ms => !ms.DestinationSupport)
                    .GetDifferentTypeMemberMappingExpression(callGetPropMethod, callSetPropMethod),
                _mappingServices.FirstOrDefault(ms => ms.DestinationSupport)
                    .GetDifferentTypeMemberMappingExpression(callGetPropMethod, callSetPropMethod));
        }
